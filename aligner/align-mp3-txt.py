...
def buildFilePathWithNewSuffix(file, suffix):
...
def align(argv):
    ...
    argv_mp3_file = sys.argv[1]
    argv_txt_file = sys.argv[2]

    init_start_time = clock()
    
    __dir__ = path.realpath(path.dirname(__file__))
    long_audio_aligner_path = __dir__ + '/long-audio-aligner'
    mp3_file = __dir__ + '/' + argv_mp3_file
    txt_file = __dir__ + '/' + argv_txt_file
    wav_file = buildFilePathWithNewSuffix(mp3_file, 'wav')
    timings_file = buildFilePathWithNewSuffix(txt_file, 'json')
